...
let toastTimer = null
const showCommentToast = (message, type = 'info') => {
...
const submitComment = async () => {
  ...
  if (!qq || !nickname || !content) {
    showCommentToast('请填写所有必填项', 'warning')
    return
  }
  if (!isValidQQ(qq)) {
    showCommentToast('请输入正确的QQ号', 'warning')
    return
  }
  ...
  try {
    ...
    if (data && data.success) {
      ...
      showCommentToast(data?.message || '评论发表失败', 'error')
    }
  } catch (e) {
    showCommentToast(e?.data?.message || e?.message || '网络错误，请稍后重试', 'error')
  } finally {
